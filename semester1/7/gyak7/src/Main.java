...
public class Main {
    ...
    static int kozepso(int a, int b, int c) {
        final int[] szamok = { a, b, c };
        Arrays.sort(szamok);
    ...
    static boolean vane7(int[] tomb) {
        for (final int elem : tomb) {
            if (elem != 0 && elem % 7 == 0) return true;
        }
        return false;
    }
    ...
    static String haziHarom(int a, int b) {
        if (a <= 0 || b <= 0) {
            throw new IllegalArgumentException("Csak pozitívak és nullánál nagyobbak lehetnek a számok");
        }
        int _lnko = lnko(a, b);
        kiir("Egyszerűsítés előtt: ", a, "/", b, " + ", b, "/", a);
        a = a / _lnko;
        b = b / _lnko;
        return a + "/" + b + " + " + b + "/" + a;
    }
